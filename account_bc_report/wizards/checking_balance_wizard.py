...
class CheckingBalanceWizard(models.TransientModel):
	...
	def _get_register_sql(self):
		...
		(
			SELECT row_number() OVER () AS id, T.mayor, T.cuenta, T.nomenclatura, T.debe, T.haber, T.saldo_deudor, T.saldo_acreedor
			FROM get_f1_register('%s','%s',%d) T
		)
