...
class AccountLetrasPaymentManual(models.Model):
	...
	def get_account(self):
		...
		for let in self:
			...
			if let.letra_payment_id.type == 'out':
				let.account_id = param.customer_letter_account_fc.id if let.currency_id.name != 'PEN' else param.customer_letter_account_nc.id
			if let.letra_payment_id.type == 'in':
				let.account_id =  param.supplier_letter_account_fc.id if let.currency_id.name != 'PEN' else param.supplier_letter_account_nc.id

	@api.onchange('imp_div','currency_id')
	def _update_debit_credit(self):
		for i in self:
			if i.imp_div and i.currency_id:
				if i.currency_id and i.currency_id.name != 'PEN':
					i.debit = i.imp_div * i.letra_payment_id.tipo_cambio if i.imp_div > 0 else 0
					i.credit = 0 if i.imp_div > 0 else abs(i.imp_div * i.letra_payment_id.tipo_cambio)
				else:
					i.debit = i.imp_div if i.imp_div > 0 else 0
