...
class AccountBankRep(models.TransientModel):
	...
	account_ids = fields.Many2many('account.account','account_book_account_bank_rel','id_bank_origen','id_account_destino',string=u'Cuentas', required=True)
	...
	def get_report(self):
		...
		self.env.cr.execute("""
			CREATE OR REPLACE view account_bank_book as ("""+self._get_sql()+""")""")
		...
		if self.type_show == 'csv':
			...
			if self.account_ids:
				...
				sql_query = """	COPY (select * from account_bank_book)TO '"""+direccion+docname+"""'   WITH DELIMITER ',' CSV HEADER			
							"""
	...
	def get_excel(self,filtro):
		...
		HEADERS = ['PERIODO','FECHA','LIBRO','VOUCHER','CUENTA','DEBE','HABER','BALANCE','SALDO MN','MON','TC','IMPORTE ME', 'SALDO ME'
		'CTA ANALITICA','GLOSA','TDP','RUC','PARTNER','TD','NRO COMP','FECHA DOC','FECHA VEN']
		worksheet = ReportBase.get_headers(worksheet,HEADERS,0,0,formats['boldbord'])
		x=1

		for line in self.env['account.bank.book'].search(filtro):
			...
			worksheet.write(x,11,line.importe_me if line.importe_me else '0.00',formats['numberdos'])
		...
		widths = [9,9,7,11,8,10,10,10,10,5,7,11,11,13,47,4,11,40,3,16,12,12]
