...
class AccountFeeRep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
			CREATE OR REPLACE view account_fee_book as ("""+self._get_sql(self.date_ini,self.date_end,self.company_id.id)+""")""")
	...
	def get_servidores(self):
		return self.get_plame(1,self.date_ini,self.date_end,self.company_id)

	def get_recibos(self):
		return self.get_plame(2,self.date_ini,self.date_end,self.company_id)

	def get_plame(self,type,x_date_ini,x_date_end,x_company):

		self.env.cr.execute("""
			CREATE OR REPLACE view account_fee_book as ("""+self._get_sql(x_date_ini,x_date_end,x_company.id)+""")""")
	...
	def get_excel(self):
		...
		HEADERS = ['PERIODO','LIBRO','VOUCHER','FECHA E','FECHA P','TD','SERIE','NUMERO','TDP','RUC','AP. PATERNO',
				   'AP. MATERNO','NOMBRES','DIVISA','MONTO ME','TC','RENTA','RETENCION','NETO P','PERIODO P','NO DOMICILIADO']
		...
		for line in self.env['account.fee.book'].search([]):
			...
			worksheet.write(x,14,line.monto_me if line.monto_me else '0.00',formats['numberdos'])
			worksheet.write(x,15,line.tipo_c if line.tipo_c else '0.0000',formats['numbercuatro'])
			worksheet.write(x,16,line.renta if line.renta else '0.00',formats['numberdos'])
			worksheet.write(x,17,line.retencion if line.retencion else '0.00',formats['numberdos'])
			worksheet.write(x,18,line.neto_p if line.neto_p else '0.00',formats['numberdos'])
			worksheet.write(x,19,line.periodo_p if line.periodo_p else '',formats['especial1'])
			worksheet.write(x,20,line.is_not_home if line.is_not_home else '',formats['especial1'])
			x += 1

		widths = [9,7,11,9,9,4,5,10,4,11,10,10,15,5,11,7,12,12,12,9,15]
	...
	def _get_sql(self,x_date_ini,x_date_end,x_company_id):
		...
		sql = """select *
			from vst_recxhon_1_1
			where (fecha_e between '%s' and '%s')
			and company_id = %s 
