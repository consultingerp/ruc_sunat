...
class StockPicking(models.Model):
	...
	serie_guia = fields.Many2one('ir.sequence', string=u'Serie de guia')
	...
	driver_id = fields.Many2one('res.partner', string='Conductor',related='vehicle_id.driver_id',readonly=False,domain=[('is_company', '=', False)])
	starting_point = fields.Char(string=u'Punto de Partida', related='picking_type_id.warehouse_id.partner_id.street', readonly=False)
	ending_point = fields.Char(string=u'Punto de Llegada',related='partner_id.street', readonly=False)
	reason_transfer = fields.Many2one('einvoice.catalog.20',string=u'Motivo de Translado')
	...
	def print_remision(self):
		...
		datos_transporte.append([Paragraph('',style_left),
							Paragraph(self.carrier_id.name if self.carrier_id else '',style_left),
